"""Standalone launcher for the structure-tensor explorer.

The UI follows the speed and coherent-flow explorers: select an existing feature
cache, inspect one replicate or the pooled overview, scrub/play the source video,
and compare block overlays with aligned time-series plots.  The first open
extracts intensity/change/appearance channels into a cache-local sidecar; later
opens reuse it.

Run:
    .venv\\Scripts\\python.exe scripts/structure_tensor_explorer.py
    .venv\\Scripts\\python.exe scripts/structure_tensor_explorer.py --cache <key>
    .venv\\Scripts\\python.exe scripts/structure_tensor_explorer.py --cache-root ./.cache
"""
...
def main() -> int:
    ...
    try:
        ...
        window = StructureTensorExplorer(
            cache, video_path, sidecar_path=sidecar)
        window.show()
        return app.exec()
    finally:
        cache.close()
