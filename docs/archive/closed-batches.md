# Archive — closed batch specs

A running log. When a batch closes, its full spec moves here from `todo.md` and
leaves a one-line row in that file's Done table. Nothing here is a plan; these
are the specs as written *before* the work, kept because several of them were
wrong in instructive ways and the correction is only legible against the original.

**The durable output of a closed batch is `FINDINGS.md`, not this file.** Read a
spec here only to answer "what did we think we were building, and how did that
differ from what we built" — never to re-derive a measurement, which is what
`FINDINGS.md` is for.

Archived 2026-07-20: batches D, E, F, I, J, O.
